...
/*   Updated: 2024/07/11 17:23:39 by eahn             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *handle_quotes(const char *input, char quote_type, int *i, int *is_double_quote)
{
    int start;
    int length;
    char *quoted_str;
    ...
    *is_double_quote = (quote_type == '"');
    while (input[*i] != quote_type && input[*i] != '\0')
        (*i)++;
    if (input[*i] == '\0')
        return (NULL); // Error: unmatched quote //todo: error handling with stderr
    length = *i - start;
...
void handle_quote_token(const char *input, t_token *tokens, int *token_count, int *i, char quote_type)
{
   int is_double_quote;
   char *quoted_str;
   quoted_str = handle_quotes(input, quote_type, i, &is_double_quote);
   if (!quoted_str)
        tokens[(*token_count)++] = create_token(TOKEN_ERROR, "Unmatched quote", 15);
    else
    {
        if (quote_type == '\'')
            tokens[(*token_count)++] = create_token (TOKEN_SINGLE_QUOTE, quoted_str, strlen(quoted_str));
        else if (is_double_quote)
            tokens[(*token_count)++] = create_token (TOKEN_DOUBLE_QUOTE, quoted_str, strlen(quoted_str));
        free(quoted_str);
    }
}


void handle_special_tokens (const char *input, t_token *tokens, int *token_count, int *i)
{
    if (input[*i] == '|')
...
void handle_token_creation(const char *input, t_token *tokens, int *token_count, int *i)
{
    int start;
    ...
    while (input[*i])
    {
        ...
        else
        {
            ...
            while(!is_delimiter(input[*i]))
                (*i)++;
            tokens[(*token_count)++] = create_token(TOKEN_ARGUMENT, input + start, *i - start);
        }
    }
}
